...
bool check_balanced(binaryTreeNode<int>* root){//bad complexity of O(n*tree_height)
    if(root == NULL){
        return true;
    }
    int a = abs(height_of_tree(root->left) - height_of_tree(root->right));
    if(a > 1){
        return false;
    }
    else{
        bool x = check_balanced(root->left);
        bool y = check_balanced(root->right);
        return x && y;
    }
}
